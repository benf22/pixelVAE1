        ...
        with tf.device(device):
            ...
            def ResidualBlock(name, input_dim, output_dim, inputs, filter_size, mask_type=None, resample=None, he_init=True):
                ...
                if mask_type == None:
                    ...
                    axes = [0, 2, 3]
                    if device_index == 0:
                        output = lib.ops.batchnorm.Batchnorm(name+'.BN', axes, output, bn_is_training, bn_stats_iter)
                    else:
                        output = lib.ops.batchnorm.Batchnorm(name+'.BN', axes, output, bn_is_training, bn_stats_iter, update_moving_stats=False)
                else:
                    output = nonlinearity(output)
            ...
            if EMBED_INPUTS:
                ...
                # embedded_images = tf.reshape(embedded_images, [-1, HEIGHT, WIDTH, DIM_EMBED*N_CHANNELS])
