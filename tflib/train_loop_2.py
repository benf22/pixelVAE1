...
def train_loop(
    ...
    def log(outputs, test, _vars, extra_things_to_print):
        ...
        print(print_str[:-1].encode("utf-8").decode("ascii"))  # omit the last \t
    ...
    while True:
        ...
        try:
            input_vals = next(train_generator)
        except StopIteration:
            train_generator = train_data()
            input_vals = next(train_generator)
            next(train_generator)
            ...
            if inject_iteration:
                ...
                if bn_vars is not None: # If using batchnorm, run over a bunch of training data first to make the running-average stats good.
                    ...
                    for i in range(bn_stats_iters):
                        try:
                            bn_stats_fn([np.int32(_vars['iteration'])] + list(next(_train_gen)), i)
                        except StopIteration:
                            _train_gen = train_data()
                            bn_stats_fn([np.int32(_vars['iteration'])] + list(next(_train_gen)), i)
                ...
                if bn_vars is not None: # If using batchnorm, run over a bunch of training data first to make the running-average stats good.
                    ...
                    for i in range(bn_stats_iters):
                        try:
                            bn_stats_fn(list(next(_train_gen)), i)
                        except StopIteration:
                            _train_gen = train_data()
                            bn_stats_fn(list(next(_train_gen)), i)
