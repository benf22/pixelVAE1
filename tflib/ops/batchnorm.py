...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True):
    if axes == [0,2,3]:
        ...
        shape_index = 1 # 3 is for NHWC and 1 is for NCHW
        offset = lib.param(name+'.offset', np.zeros(inputs.get_shape()[shape_index], dtype='float32'))
        scale = lib.param(name+'.scale', np.ones(inputs.get_shape()[shape_index], dtype='float32'))

        moving_mean = lib.param(name+'.moving_mean', np.zeros(inputs.get_shape()[shape_index], dtype='float32'), trainable=False)
        moving_variance = lib.param(name+'.moving_variance', np.ones(inputs.get_shape()[shape_index], dtype='float32'), trainable=False)

        def _fused_batch_norm_training():
            return tf.nn.fused_batch_norm(inputs, scale, offset, epsilon=1e-2, data_format='NCHW')#''NHWC')
        def _fused_batch_norm_inference():
            # Version which blends in the current item's statistics
            batch_size = tf.cast(tf.shape(inputs)[0], 'float32')
            mean, var = tf.nn.moments(inputs, [2,3], keep_dims=True) #This is for NCHW
        ...
        if is_training is None:
            raise Exception('no is_training')
